package com.example.CiCdPipelineWithExcel;
...
public class ExcelMonitor {
	public static void main(String[] args) {
		...
		String filePath = // "D:\\Xaltius(New)\\Material\\NTUC\\CNATIVE\\DevOpsIntroduction\\ScienceFair.xlsx"; // Path to the Excel file
		"C:\\Zzzz\\FullStackDeveloper\\18-DevOpsIntro-WkCicdContMonitoringToolsLearnerMaterials\\Hands On\\20Jun2025\\CiCdPipelineWithExcel\\src\\main\\java\\com\\example\\CiCdPipelineWithExcel\\ScienceFair.xlsx";
		try (FileInputStream fis = new FileInputStream(filePath); Workbook workbook = new XSSFWorkbook(fis)) {
			...
			for (int i = 1; i <= sheet.getLastRowNum(); i++) {
				...
				if (!isCellEmpty(avgCell) && judge1 >= 0 && judge2 >= 0) {
					...
					if (avg != correctAvg) {
						System.out.println("Row " + (i + 1) + ": Incorrect average score. Your value: "+avgCell+" Expected: " + correctAvg);
						errorsFound = true;
					}
				}
			}
